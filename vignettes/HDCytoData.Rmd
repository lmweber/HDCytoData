...
Currently, the package contains the following datasets, which can be grouped into datasets useful for benchmarking either (i) clustering algorithms or (ii) methods for differential analysis.
...
Note that flow and mass cytometry data should be transformed prior to performing any downstream analyses, such as clustering. Standard transforms include the \code{\link{asinh}} with \code{cofactor} parameter equal to 5 (for mass cytometry data) or 150 (for flow cytometry data).
