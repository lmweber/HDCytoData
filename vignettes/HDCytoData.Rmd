...
We welcome contributions or suggestions for new datasets to include in the `HDCytoData` package. Suitable datasets are any high-dimensional cytometry datasets that are useful for benchmarking, and contain some sort of ground truth or known signal (e.g. a known ground truth in simulated data, or a known biological result in experimental data).

To contribute a dataset, the following are required:

- formatted `SummarizedExperiment` and `flowSet` objects containing table(s) of expression values and all required row, column, and other metadata
- reproducible R scripts showing how the formatted objects were generated from the original raw data files (for examples, see the `.R` files in the directory `inst/scripts` in the source code of the `HDCytoData` package)
- comprehensive documentation (for examples, see the `.Rd` files in the directory `man` in the source code of the `HDCytoData` package)

Contributions or suggestions can be provided via GitHub issues on the [HDCytoData GitHub page](https://github.com/lmweber/HDCytoData).

For more details on how to contribute resources to `ExperimentHub`, see the `ExperimentHub` vignettes available from [Bioconductor](http://bioconductor.org/packages/ExperimentHub).
